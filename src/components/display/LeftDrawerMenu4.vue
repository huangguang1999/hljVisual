  ...
  methods: {
    selectGet (vId) {
      ...
      if (areaObj.name === '黑龙江省') {
        ...
        this.valueTime = ''
        this.valuedataElement = ''
        this.time = [ConstData.Time[3]]
      } else {
        this.time = ConstData.Time
      }
    ...
    // 刷新x轴数据
    refreshTableDate () {
      this.$store.dispatch('refreshFun4', this.resOption)
      this.$store.dispatch('refreshTitleFun4', this.areaChoose.name + this.timeChoose.name + this.dataElementChoose.name)
    },
    refreshTableDate1 () {
      this.$store.dispatch('refreshXDateFun4', '年度')
    },
    refreshTableDate2 () {
      this.$store.dispatch('refreshXDateFun4', '地区')
    },
    refreshTableDate3 () {
      this.$store.dispatch('refreshXDateFun4', 'city')
    },
    refreshTableDate4 () {
      this.$store.dispatch('refreshXDateFun4', this.dataElementChoose)
    },
    // 确定事件
    pushForm: function () {
